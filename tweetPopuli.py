...
def tweet_sentiment(tweet):
    '''
    using a NLP library textblob method, return if the tweet was considered neg or pos
    '''
    analysis = TextBlob(tweet)
    if analysis.sentiment.polarity > .10:
        return 'Positive'
    else:
        return 'Negative'
...
def getReplies(username, user_tweets):
    '''
    get replies of a politcians tweet, take sentiment of each response
    '''
    responses_to_user = []
...
def pos_neg_sort(replies_to_user, name):
    '''
    sort through replies, put them into negative or positive tweets
    '''
    ptweets = []
    ntweets = []
...
def get_tweets_for_user(user_screen_name):
    '''
    getting the actual tweet for each user
    '''
    screen_name_tweets = []
    screen_name_tweets = api.user_timeline(screen_name = user_screen_name, count = 1, include_rts = False)
    return screen_name_tweets

politician_list = ["realDonaldTrump", "AOC","JoeBiden", "SenWarren","BorisJohnson","marcorubio"] 
